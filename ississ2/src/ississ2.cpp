    ...
    const struct Iss2::DataResponse &drsp, uint32_t irq_bit_field )
{
...

    {
        ...
        if ( busy || ! m_i_access_ok || ! m_d_access_ok ) {
            if(busy) std::cerr << "busy ";
            if(!m_i_access_ok) std::cerr << "!m_i_access_ok ";
            if(!m_d_access_ok) std::cerr << "!m_d_access_ok";
            std::cerr << "\n";
...
tmpl(int)::debugCpuCauseToSignal( uint32_t cause ) const
{
	...
	/************************************************************************/
	/* Methods required by CDB                                        START */
	/************************************************************************/
	
tmpl(const char *)::local_GetModel()
{
    return(m_iss.local_GetModel());
}

tmpl(int)::local_PrintResource(modelResource *res,char **p)
{
    int ret;
    ret = m_iss.local_PrintResource(res,p);
    return(ret);
}

tmpl(int)::local_TestResource(modelResource *res,char **p)
{
    int ret;
    ret = m_iss.local_TestResource(res,p);
    return(ret);
}

tmpl(int)::local_Resource(char **args)
{
    return(m_iss.local_Resource(args));
}

	/************************************************************************/
	/* Methods required by CDB                                          END */
	/************************************************************************/
